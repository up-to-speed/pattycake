...
const __result = match(fileChangesState)
      .with(
        { type: "loading" },
        () => diffViewerConfig.overscan.overscanBounds.min,
      )
      .with({ type: "ready" }, ({ data }) =>
        decideOverscan(
          diffViewerConfig.overscan,
          data.map((f) => f.diff.oldLines.length),
        ),
      )
      .exhaustive();