...
const __result = match(result.exitCode)
    .with(GrepLikeExitCode.ERROR, (): SearchResult => {
    ...
    .with(GrepLikeExitCode.NO_MATCHES, (): SearchResult => {
      logger.debug("handleSearchToolExitCode: No matches found", { query });
    ...
    .otherwise((): SearchResult => {
      ...
      logger.error(
        `handleSearchToolExitCode: Unexpected ${toolName} exit code`,
        {
          exitCode: result.exitCode,
        },
      );
