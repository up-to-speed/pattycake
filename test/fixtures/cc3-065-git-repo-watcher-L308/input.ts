...
const __result = match(repoKind)
      .with({ type: "root" }, async () => {
        ...
        return { type: "root" as const, fetchInterval };
      })
      .with({ type: "clone" }, async (repoKind) => {
        const unsubscribeFromRoot = await this.subscribe(
          repoKind.rootRepoPath,
          {
            onFilesChanged: () => {},
        ...
        void runAsyncSwallowingErrors(
          "GitRepoWatcher: Error in initial ref sync",
          () => this.syncCloneRefsFromRoot(repoPath, repoKind.rootRepoPath),
        );

        logger.debug(
          `GitRepoWatcher: Registered clone ${repoPath} → root ${repoKind.rootRepoPath}`,
        );

        return { ...repoKind, unsubscribeFromRoot };
      })
      .exhaustive();