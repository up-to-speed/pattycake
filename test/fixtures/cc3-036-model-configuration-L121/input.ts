...
const __result = match(provider)
    .with(ModelProvider.ANTHROPIC, async () => {
      ...
      try {
        ...
        if (!model) {
          logger.debug(
            "[Model Config] Claude Code CLI not available: executable not found",
          );
          return false;
        }
    ...
    .with(ModelProvider.GOOGLE, async () => {
      ...
      const credPaths = [
        path.join(homeDir, ".gemini", "mcp-oauth-tokens-v2.json"),
      ...
      try {
        ...
        const model = geminiProvider.languageModel(
          "gemini-2.5-flash",
        ) as LanguageModelV2 & { specificationVersion: "v3" };
        ...
        logger.debug("[Model Config] Gemini CLI not available", { error });
        return false;
      }
    })
