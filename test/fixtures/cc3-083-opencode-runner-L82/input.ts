...
const __result = match(error)
    .with({ name: "APIError" }, (e: ApiError) => e.data.message)
    .with(
      { name: "ProviderAuthError" },
      (e: ProviderAuthError) => e.data.message,
    )
    .with({ name: "NotFoundError" }, (e: NotFoundError) => e.data.message)
    .with({ success: false as const }, (e: BadRequestError) =>
      e.errors
        .map((err: Record<string, unknown>) => JSON.stringify(err))
        .join("; "),
    )
    .otherwise(() => {
      ...
      if (typeof error === "object" && error !== null) {
        ...
        const status =
          typeof record.statusCode === "number"
            ...
            : record.status;
        if (message)
          return status !== null ? `${message} (status: ${status})` : message;
